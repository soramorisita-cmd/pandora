...
MOBILE_APP_JS = """<script>(function(){var ua=navigator.userAgent;var isIOS=/iPhone|iPad|iPod/i.test(ua);var isAnd=/Android/i.test(ua);if(!isIOS&&!isAnd)return;var isInApp=/Discord/i.test(ua)||(isIOS&&!/Safari\//i.test(ua))||(isAnd&&(/wv\)/i.test(ua)||(/Version\/\d/i.test(ua)&&/Chrome/i.test(ua))));function tryApp(webUrl){var appUrl=webUrl.replace('https://www.kakobuy.com/','kakobuy://');if(isAnd){window.location.href='intent://'+appUrl.replace('kakobuy://','')+'#Intent;scheme=kakobuy;S.browser_fallback_url='+encodeURIComponent(webUrl)+';end';}else{var t=setTimeout(function(){window.location.href=webUrl;},1500);var onVis=function(){clearTimeout(t);document.removeEventListener('visibilitychange',onVis);};document.addEventListener('visibilitychange',onVis);var ifr=document.createElement('iframe');ifr.style.cssText='position:fixed;width:1px;height:1px;opacity:0;top:-1px';document.body.appendChild(ifr);ifr.src=appUrl;setTimeout(function(){if(ifr.parentNode)ifr.parentNode.removeChild(ifr);},2000);}}document.addEventListener('click',function(e){var a=e.target.closest('a.btn-buy,a.btn-primary');if(!a||!a.href||!a.href.includes('kakobuy.com/item'))return;e.preventDefault();if(isInApp){window.location.href=a.href;return;}tryApp(a.href);});})()</script>"""
